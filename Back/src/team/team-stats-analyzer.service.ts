...
export class TeamStatsAnalyzerService {
  ...
  async analyzeTeamStats(gameData: any): Promise<TeamStatsResult> {
    ...
    const homeTeamStats: TeamStatsData = this.createEmptyStats(
      gameData.homeTeam || 'Home',
    );
    const awayTeamStats: TeamStatsData = this.createEmptyStats(
      ...
      if (currentDown === 3) {
        ...
        if (i + 1 < gameData.Clips.length) {
          ...
          if (
            ...
            console.log(`  ❌ 3rd down 실패 (팀 변경 또는 다운 != 1)`);
          }
        }
      }
    }
  ...
  private analyzeClip(
    ...
    if (
      significantPlays.includes('INTERCEPT') &&
      significantPlays.includes('TURNOVER')
    ) {
      ...
      console.log(
        `   🔥 팀 인터셉트 기록! 팀: ${isHomeOffense ? '홈' : '어웨이'}`,
      );
    ...
    if (
      ...
      defenseStats.opponentTurnovers += 1;
      console.log(
        `   🔥 팀 펌블 로스트 기록! 팀: ${isHomeOffense ? '홈' : '어웨이'}`,
      );
    }
    ...
    if (
      ...
      if (
        significantPlays.includes('FIELDGOALGOOD') ||
    ...
    const isPenalty = significantPlays.some(
      (play) =>
        play &&
    ...
    if (isPenalty) {
      // 해당 팀의 페널티인지 확인
      const isHomePenalty = significantPlays.some(
        (play) => play && play.includes('PENALTY.HOME'),
      );
      const isAwayPenalty = significantPlays.some(
        (play) => play && play.includes('PENALTY.AWAY'),
      );
      ...
      if (
        ...
        offenseStats.penaltyYards += penaltyYards;
      } else if (
        (isHomeOffense && isAwayPenalty) ||
        (!isHomeOffense && isHomePenalty)
      ) {
        ...
        defenseStats.penaltyYards += penaltyYards;
      }
    }
  }
  ...
  private async updateTeamTotalStats(
    ...
    if (existingStats) {
      ...
      const newTeamStats = new this.teamTotalStatsModel({
        ...
        thirdDownAttempts: this.safeNumber(teamStats.thirdDownAttempts),
        thirdDownMade: this.safeNumber(teamStats.thirdDownMade),
        gamesPlayed: 1,
        wins: 0,
        losses: 0,
        ties: 0,
  ...
  async analyzeGameForDisplay(gameData: any) {
    ...
    console.log('홈팀 스탯:', {
      ...
      '3rd다운': `${homeTeamStats.thirdDownMade}/${homeTeamStats.thirdDownAttempts}`,
    });
    console.log('어웨이팀 스탯:', {
      ...
      '3rd다운': `${awayTeamStats.thirdDownMade}/${awayTeamStats.thirdDownAttempts}`,
    });
      ...
      if (teamTotalStats.length > 0) {
        ...
        const formattedStats = teamTotalStats
          .map((team) => ({
            ...
            avgRushingYardsPerGame:
              team.gamesPlayed > 0
                ? Math.round((team.rushingYards || 0) / team.gamesPlayed)
                : 0,
            avgRushingYardsPerCarry:
              (team.rushingAttempts || 0) > 0
            ...
            avgPassingYardsPerGame:
              team.gamesPlayed > 0
                ? Math.round((team.passingYards || 0) / team.gamesPlayed)
                : 0,
            avgPassingYardsPerAttempt:
              (team.passAttempts || 0) > 0
