...
async function generateTeamStats() {
  ...
  try {
    ...
    for (const gameData of allGames) {
      ...
      console.log(
        `  홈팀: ${gameData.homeTeam} vs 어웨이팀: ${gameData.awayTeam}`,
      );

      try {
        // 팀 통계 분석
        const teamStatsResult =
          await teamStatsService.analyzeTeamStats(gameData);
    ...
    allTeamStats.forEach((team, index) => {
      console.log(
        `${index + 1}. ${team.teamName}: 총 ${team.totalYards}야드 (${team.gamesPlayed}경기)`,
      );
    });
  } catch (error) {
