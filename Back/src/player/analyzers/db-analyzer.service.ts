...
export class DbAnalyzerService extends BaseAnalyzerService {
  ...
  async analyzeClips(clips: ClipData[], gameData: GameData): Promise<any> {
    ...
    for (const [dbKey, dbStats] of dbStatsMap) {
      ...
      console.log(
        `🚨 DB ${dbStats.jerseyNumber}번 (${dbStats.teamName}) 최종 스탯:`,
      );
      ...
      console.log(
        `   킥오프 리턴: ${dbStats.kickoffReturn}회, ${dbStats.kickoffReturnYard}야드, TD: ${dbStats.kickoffReturnTouchdowns}`,
      );
      console.log(
        `   펀트 리턴: ${dbStats.puntReturn}회, ${dbStats.puntReturnYard}야드, TD: ${dbStats.puntReturnTouchdowns}`,
      );
      ...
      const saveResult = await this.savePlayerStats(
        ...
        'DB',
        {
          ...
          returnTouchdowns:
            dbStats.kickoffReturnTouchdowns + dbStats.puntReturnTouchdowns,
        },
        gameData,
      );
   ...
   * 개별 클립을 DB 관점에서 처리
   */
  private processClipForDB(
    clip: ClipData,
    dbStatsMap: Map<string, DBStats>,
    gameData: GameData,
  ): void {
    ...
    console.log(
      `   🔍 클립 ${clip.clipKey}: playType=${clip.playType}, car=${clip.car?.num}(${clip.car?.pos}), tkl=${clip.tkl?.num}(${clip.tkl?.pos})`,
    );
    ...
    for (const dbPlayer of dbPlayers) {
      ...
      if (!dbStatsMap.has(dbKey)) {
        ...
        dbStatsMap.set(
          dbKey,
          this.initializeDBStats(dbPlayer.number, teamName),
        );
  ...
  private processPlay(
    ...
    if (playerRole === 'tkl' || playerRole === 'tkl2') {
      ...
      if (
        ...
        console.log(
          `   🟢 DB 펌블 리커버리: ${Math.abs(clip.gainYard || 0)}야드`,
        );
      }
