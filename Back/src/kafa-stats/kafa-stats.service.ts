...
export class KafaStatsService {
  ...
  async getTeamStats(
    ...
    try {
      ...
      this.logger.log(
        `Successfully fetched ${stats.length} team stats for ${league}`,
      );
      return stats;
    } catch (error) {
    ...
    try {
      ...
      $('.stats_table tr').each((index, element) => {
        ...
        if (cells.length >= 6) {
          ...
          this.logger.log(
            `🧐 셀 분석: [${cell0}] [${cell1}] [${cell2}] [${cell3}] [${cell4}] [${cell5}]`,
          );

          // 선수 정보가 두 번째 셀에 있을 가능성
          const playerCell = cell1.includes('대학교')
            ? cell1
            : cell0.includes('대학교')
              ? cell0
              : cell2.includes('대학교')
          ...
          const { playerName, university, jerseyNumber } =
            this.parsePlayerInfo(playerCell);

          const rushYards = cell1.includes('대학교') ? cell2 : cell1;
          const yardsPerAttempt =
            parseFloat(cell1.includes('대학교') ? cell3 : cell2) || 0;
          const attempts =
            parseInt(cell1.includes('대학교') ? cell4 : cell3) || 0;
          const touchdowns =
            parseInt(cell1.includes('대학교') ? cell5 : cell4) || 0;
          const longest =
            parseInt(
              cell1.includes('대학교') ? $(cells[6])?.text().trim() : cell5,
            ) || 0;
      ...
      this.logger.log(
        `Successfully fetched ${stats.length} player stats for ${league}`,
      );
      return stats;
    } catch (error) {
  ...
  // 선수 정보 파싱 헬퍼 함수
  private parsePlayerInfo(text: string): {
    playerName: string;
    university: string;
    jerseyNumber: number;
  } {
    ...
    if (match) {
      ...
      this.logger.log(
        `✅ 파싱 성공: ${university} ${jerseyNum}번 ${playerName}`,
      );
      return {
    ...
    if (match2) {
      ...
      this.logger.log(
        `✅ 대안 파싱 성공: ${university.trim()} ${jerseyNum}번 ${playerName.trim()}`,
      );
      return {
        playerName: playerName.trim(),
        university: university.trim(),
  ...
  async getTeamPlayerStats(
    ...
    return allStats.filter(
      (stat) =>
        stat.university.includes(teamName) ||
        teamName.includes(stat.university),
    );
  }

  // 특정 선수 스탯 조회
  async getSpecificPlayerStats(
    league: 'uni' | 'soc',
    playerName: string,
    year?: string,
    ...
    return (
      allStats.find(
        (stat) =>
          stat.playerName.includes(playerName) ||
          playerName.includes(stat.playerName),
      ) || null
    );
  }
  ...
  async savePlayerStatsToDB(
    ...
    try {
      ...
      for (const stat of playerStats) {
        ...
        this.logger.log(
          `✅ 저장 완료: ${stat.university} ${stat.jerseyNumber}번 ${stat.playerName}`,
        );
      }
  ...
  async debugKafaDB(): Promise<number> {
    try {
      ...
      // 최근 5개 샘플 데이터 조회
      const sampleData = await this.kafaPlayerStatsModel.find().limit(5).exec();
  ...
  async exploreAllStatPages(league: 'uni' | 'soc' = 'uni'): Promise<any> {
    ...
    for (let i = 1; i <= 11; i++) {
      try {
        ...
        // 페이지 제목 추출
        const pageTitle =
          $('h1, h2, .page-title').first().text().trim() ||
          $('title').text().trim();
  ...
  async crawlStatPage(
    ...
    try {
      ...
      this.logger.log(
        `✅ 페이지 ${pageNumber}에서 ${stats.length}명의 선수 데이터 크롤링 완료`,
      );
      return stats;
    } catch (error) {
      this.logger.error(
        `❌ 페이지 ${pageNumber} 크롤링 실패: ${error.message}`,
      );
  ...
  async loginToKafa(username: string, password: string): Promise<boolean> {
    try {
      ...
      if (
        loginResponse.data.includes('로그아웃') ||
  ...
  async crawlMatchData(matchId: number): Promise<any> {
    try {
      ...
      this.logger.log(
        `📊 경기 데이터 크롤링 (기록용지 포함): Match ID ${matchId}`,
      );
      ...
      $('input').each((index, element) => {
        ...
        this.logger.log(
          `  Input ${index + 1}: name="${name}", type="${type}", value="${value}", id="${id}"`,
        );
      });
      ...
      $('select').each((index, element) => {
        ...
        const id = $(element).attr('id');
        this.logger.log(
          `  Select ${index + 1}: name="${name}", value="${selectedValue}", text="${selectedText}", id="${id}"`,
        );

        // select의 모든 option도 출력
        $(element)
          .find('option')
          .each((optIndex, option) => {
            const optValue = $(option).attr('value');
            const optText = $(option).text().trim();
            this.logger.log(
              `    Option ${optIndex + 1}: value="${optValue}", text="${optText}"`,
            );
          });
      });
      ...
      // 텍스트 내용 샘플
      this.logger.log(
        `🔍 페이지 텍스트 샘플: ${$('body').text().substring(0, 200)}...`,
      );
      ...
      const teamIdMatches = response.data.match(
        /team_cat\('\d+','(\d+)','home'\).*?team_cat\('\d+','(\d+)','away'\)/,
      );
      ...
      const matchData = {
        ...
        // 추가 URLs (상세 리포트 링크들)
        reportUrls: {
          homeTeam2ndReport: homeTeamId
            ? `https://www.kafa.org/SCORES/report_2nd.html?result_idx=${matchId}&team_idx=${homeTeamId}`
            : null,
          awayTeam2ndReport: awayTeamId
            ? `https://www.kafa.org/SCORES/report_2nd.html?result_idx=${matchId}&team_idx=${awayTeamId}`
            : null,
          homeTeam3rdReport: homeTeamId
            ? `https://www.kafa.org/SCORES/report_3rd.html?result_idx=${matchId}&team_idx=${homeTeamId}`
            : null,
          awayTeam3rdReport: awayTeamId
            ? `https://www.kafa.org/SCORES/report_3rd.html?result_idx=${matchId}&team_idx=${awayTeamId}`
            : null,
      ...
      try {
        ...
        this.logger.warn(
          `⚠️ quarterPlayData JSON 변환 실패, 요약 정보만 포함: ${jsonError.message}`,
        );
      ...
      this.logger.log(
        `✅ 전체 경기 데이터 크롤링 완료: ${matchData.meta.totalInputs}개 input, ${matchData.meta.totalSelects}개 select, ${matchData.meta.totalTables}개 테이블`,
      );
      return matchData;
    } catch (error) {
  ...
  async crawlQuarterPlayData(matchId: number): Promise<any> {
    ...
    try {
      for (const qtr of quarters) {
        ...
        this.logger.log(
          `✅ ${qtr.toUpperCase()} 데이터 수집 완료: ${playData?.plays?.length || 0}개 플레이`,
        );
      }
      ...
      this.logger.error(
        `❌ 쿼터별 플레이 데이터 크롤링 실패: ${error.message}`,
      );
      return {};
    }
  }
  ...
  async getQuarterData(matchId: number, quarter: string): Promise<any> {
    try {
      ...
      this.logger.log(
        `📥 ${quarter.toUpperCase()} 응답 타입: ${typeof response.data}`,
      );
      this.logger.log(
        `📥 ${quarter.toUpperCase()} 응답 길이: ${response.data?.length || 0}자`,
      );
      this.logger.log(
        `📥 ${quarter.toUpperCase()} 응답 내용: ${JSON.stringify(response.data)?.substring(0, 300)}...`,
      );
      ...
      if (response.data) {
        ...
        if (
          ...
          this.logger.log(
            `✅ ${quarter.toUpperCase()} JSON 객체 확인됨 - ${response.data.count}개 플레이`,
          );
      ...
      this.logger.warn(
        `⚠️ ${quarter.toUpperCase()} 데이터 수집 실패: ${error.message}`,
      );
  ...
  private parsePlayData(data: any, quarter: string): any {
    try {
      ...
      this.logger.error(
        `❌ ${quarter} JSON 플레이 데이터 파싱 실패: ${error.message}`,
      );
      return { quarter, plays: [], count: 0, error: error.message };
    }
  }
  ...
  private parseHtmlPlayData(html: string, quarter: string): any {
    try {
      ...
      this.logger.error(
        `❌ ${quarter} HTML 플레이 데이터 파싱 실패: ${error.message}`,
      );
      return { quarter, plays: [], count: 0, error: error.message };
    }
  }
    ...
    try {
      ...
      if (index === 1) {
        ...
        cells.slice(0, 15).forEach((cell, idx) => {
          this.logger.log(
            `  셀 ${idx}: "${cell.text}" (class: ${cell.className})`,
          );
        });
      }

      // rawCells[0]에서 실제 플레이 번호 추출 (숫자만)
      const actualPlayNumber = parseInt(
        cells[0]?.text?.replace(/\D/g, '') || index.toString(),
      );
      ...
      this.logger.warn(
        `⚠️ 플레이 데이터 구조화 실패 (Row ${index}): ${error.message}`,
      );
      return null;
    }
  }
  ...
  async savePlayDataToDB(matchId: number, quarterData: any): Promise<void> {
    ...
    try {
        ...
        if (quarterPlayData?.plays && Array.isArray(quarterPlayData.plays)) {
          for (const play of quarterPlayData.plays) {
            try {
              ...
              totalErrors++;
              this.logger.warn(
                `⚠️ 플레이 데이터 저장 실패 (${quarter} Play ${play.playNumber}): ${saveError.message}`,
              );
            }
          }
        }
      ...
      this.logger.log(
        `✅ 플레이 데이터 DB 저장 완료: ${totalSaved}개 저장, ${totalErrors}개 실패`,
      );
    } catch (error) {
  ...
  async getPlayDataFromDB(matchId: number, quarter?: string): Promise<any[]> {
    try {
      ...
      this.logger.log(
        `📊 플레이 데이터 조회 완료: ${playData.length}개 플레이`,
      );
      return playData;
    } catch (error) {
  ...
  async crawlAndMergeAllStats(
    ...
    try {
      this.logger.log(
        `🚀 전체 스탯 크롤링 시작: ${league} 리그 ${season} 시즌`,
      );
  ...
  async calculateGameRecordStats(
    ...
    try {
      this.logger.log(
        `📊 경기 ${matchId} 기록지 통계 계산 시작${quarter ? ` (${quarter})` : ''}`,
      );
      ...
      const teams = [
        ...new Set(
          plays.map((play) => play.offenseTeam).filter((team) => team),
        ),
      ];
      ...
      // 쿼터별 통계 초기화
      const quarters = [
        ...new Set(plays.map((play) => play.quarter).filter((q) => q)),
      ];
      quarters.forEach((q) => {
      ...
      plays.forEach((play) => {
        ...
        if (play.playNumber <= 10) {
          this.logger.log(
            `🔍 플레이 ${play.playNumber}: ${play.offenseTeam} - playType: ${playType}`,
          );
          this.logger.log(
            `  rawCells[7]: ${play.rawCells?.[7]?.text || 'undefined'}`,
          );
          this.logger.log(`  rawCells length: ${play.rawCells?.length || 0}`);
          this.logger.log(
            `  gainYards.offense: ${play.gainYards?.offense || 'empty'}`,
          );
        }
        ...
        if (playType === 'PASS') {
          ...
          const passingYards = this.extractYards(
            play.gainYards?.offense || play.rawCells?.[8]?.text || '0',
          );
          teamStat.passingYards += passingYards;
        } else if (playType === 'RUSH') {
          teamStat.rushingPlays++;
          // 러싱 야드 계산 (gainYards.offense 사용)
          const rushingYards = this.extractYards(
            play.gainYards?.offense || '0',
          );
        ...
        // 턴오버 체크
        if (
          play.fumbleRecovery?.playerNumber ||
          play.interception?.playerNumber
        ) {
          teamStat.turnovers++;
        }
        ...
        if (play.down && play.down.startsWith('3-')) {
          ...
          if (
            (play.firstDown && play.firstDown.trim() === 'FD') ||
      ...
      plays.forEach((play) => {
        ...
        if (penaltyValue) {
          this.logger.log(
            `패널티 필드 발견: "${penaltyValue}" (${play.quarter} Play ${play.playNumber})`,
          );
          penaltyCount++;
        }
        ...
        if (
          ...
          if (penaltyTeam && stats.teamStats[penaltyTeam]) {
            ...
            // 디버그 로그
            this.logger.log(
              `✅ 패널티 카운트: ${penaltyValue} → 팀: ${penaltyTeam}, 야드: ${penaltyYards}, 플레이: ${play.quarter} ${play.playNumber}`,
            );
          } else {
            this.logger.log(
              `❌ 패널티 팀 추출 실패: ${penaltyValue} → 팀: ${penaltyTeam}`,
            );
          }
        }
      });
      ...
      this.logger.log(
        `🔍 패널티 처리 완료: 총 ${penaltyCount}개 패널티 필드 발견`,
      );
      ...
      this.logger.log(
        `✅ 경기 ${matchId} 통계 계산 완료: ${plays.length}개 플레이 분석`,
      );
      return {
        success: true,
  ...
  async getGameGeneralInfo(matchId: number): Promise<any> {
    try {
      ...
      const crawlingResultPath = path.join(
        __dirname,
        '../../../crawling_result.json',
      );
      if (fs.existsSync(crawlingResultPath)) {
        const crawlingResult = JSON.parse(
          fs.readFileSync(crawlingResultPath, 'utf8'),
        );
      ...
      const quarterDataPath = path.join(
        __dirname,
        '../../../crawling_result_qtr1.json',
      );
      if (fs.existsSync(quarterDataPath)) {
        const quarterResult = JSON.parse(
          fs.readFileSync(quarterDataPath, 'utf8'),
        );
      ...
      if (fs.existsSync(crawlingResultPath)) {
        ...
        if (crawlingResult.data.matchId === matchId) {
          ...
          this.logger.log(
            `✅ Total(자동합산) 점수 추출: 홈 ${finalScores.home.total} - 어웨이 ${finalScores.away.total}`,
          );
        }
      }
      ...
      if (finalScores) {
        ...
        this.logger.warn(
          `⚠️ Total(자동합산) 점수 없음, 플레이별 계산으로 폴백`,
        );
            ...
            quarterData.quarters[quarter].forEach((play: any) => {
              if (play.score && play.score.type) {
                ...
                if (
                  points > 0 &&
                  scoreTeam &&
                  teamScores.hasOwnProperty(scoreTeam)
                ) {
      ...
      const homeTeamFullName =
        teamNameMap[homeTeam as keyof typeof teamNameMap] ||
        `${homeTeam} 대학교`;
      const awayTeamFullName =
        teamNameMap[awayTeam as keyof typeof teamNameMap] ||
        `${awayTeam} 대학교`;
      ...
      const gameGeneralInfo = {
        ...
        quarterScores: {
          q1: {
            home: quarterScores.q1[homeTeam],
            away: quarterScores.q1[awayTeam],
          },
          q2: {
            home: quarterScores.q2[homeTeam],
            away: quarterScores.q2[awayTeam],
          },
          q3: {
            home: quarterScores.q3[homeTeam],
            away: quarterScores.q3[awayTeam],
          },
          q4: {
            home: quarterScores.q4[homeTeam],
            away: quarterScores.q4[awayTeam],
          },
        },
      ...
      this.logger.log(
        `🏆 최종 스코어: ${teamScores[homeTeam]} - ${teamScores[awayTeam]}`,
      );
  ...
  async getTeamStatsByPage(
    ...
    try {
        ...
        if (cells.length >= 3) {
          ...
          if (teamName && teamName !== '팀명') {
            ...
            cells.each((cellIndex, cell) => {
              ...
              const fieldName =
                fieldMappings[cellIndex] || `unknown${cellIndex}`;
      ...
      this.logger.error(
        `❌ 팀 스탯 페이지 크롤링 실패 (${pageType}): ${error.message}`,
      );
      return [];
    }
  }
  ...
  private getTeamStatFieldMapping(pageType: string): Record<number, string> {
    const mappings: Record<string, Record<number, string>> = {
      ...
      defense2: {
        // 인터셉션 (KAFA: INT, INT TD, INT YDS, LNG)
        1: 'interceptions', // INT
  ...
  // 개인 스탯 필드 매핑 정의
  private getPlayerStatFieldMapping(
    pageNumber: number,
    ...
    const mappings: Record<number, Record<number, string>> = {
      ...
      3: {
        // 개인 리시빙 (REC, REC YDS, YDS/ATT, TD, LNG)
        2: 'receptions',
        3: 'receivingYards',
        4: 'yardsPerReception',
        5: 'targets',
      ...
      6: {
        // 개인 인터셉션 (INT, INT TD, INT YDS, LNG)
        2: 'interceptions',
        3: 'touchdowns',
        4: 'interceptionYards',
        5: 'longestReturn',
      ...
      7: {
        // 개인 킥킹 (FG%, YDS AVG, FGM, ATT, YDS, LNG)
      ...
      9: {
        // 개인 킥오프 리턴 (YDS AVG, KO RETURNS, YDS, TD, LNG)
        2: 'yardsPerReturn',
        3: 'returns',
        4: 'returnYards',
        5: 'touchdowns',
      ...
      10: {
        // 개인 펀팅 (YDS AVG, PUNTS, YDS, TD, LNG)
        2: 'averageDistance',
        3: 'totalPunts',
        4: 'totalYards',
        5: 'touchdowns',
      ...
      11: {
        // 개인 펀트 리턴 (YDS AVG, PUNT RETURNS, YDS, TD, LNG)
        2: 'yardsPerReturn',
        3: 'returns',
        4: 'returnYards',
        5: 'touchdowns',
  ...
  async getPlayerStatsByPage(
    ...
    try {
      ...
      this.logger.log(
        `✅ 개인 스탯 페이지 ${pageNumber} - ${stats.length}개 크롤링 완료`,
      );
      return stats;
    } catch (error) {
      this.logger.error(
        `❌ 개인 스탯 페이지 크롤링 실패 (페이지 ${pageNumber}): ${error.message}`,
      );
      return [];
    }
  }
  ...
  async getAllKafaStats(year?: string): Promise<any> {
    try {
      this.logger.log(
        `🚀 전체 KAFA 통계 데이터 수집 시작 (년도: ${year || 'current'})`,
      );
  ...
  async getPlayersFromJson(league: 'uni' | 'soc' = 'uni'): Promise<{
    ...
    try {
      ...
      this.logger.log(
        `📖 JSON 파일에서 ${league} 선수 데이터 조회: ${jsonData.totalCount}명`,
      );
  ...
  async scrapeAllStatsToJson(league: 'uni' | 'soc' = 'uni'): Promise<{
    ...
    try {
      ...
      // 11개 스탯 타입 순차적으로 크롤링
      for (const [pageNum, statInfo] of Object.entries(
        this.STAT_TYPE_MAPPING,
      )) {
        try {
          this.logger.log(
            `🔄 ${statInfo.name} 크롤링 중... (페이지 ${pageNum})`,
          );

          const players = await this.getPlayerStatsByPage(
            league,
            parseInt(pageNum),
          );

          if (players && players.length > 0) {
            const saveResult = await this.saveStatTypeToJson(
              league,
              statInfo.key,
              statInfo.name,
              players,
            ...
            this.logger.log(
              `✅ ${statInfo.name}: ${players.length}명 저장 완료`,
            );
          } else {
            results.push({
              statType: statInfo.key,
          ...
          this.logger.error(
            `❌ ${statInfo.name} 크롤링 실패: ${error.message}`,
          );
          results.push({
            statType: statInfo.key,
            success: false,
      ...
      this.logger.log(
        `✅ 전체 크롤링 완료: ${successCount}/11개 스탯, 총 ${totalPlayers}명`,
      );
  ...
  async getStatTypeFromJson(
    ...
    try {
      ...
      this.logger.log(
        `📖 ${league} ${statType} 데이터 조회: ${jsonData.totalCount}명`,
      );
  ...
  async getAllStatFiles(league?: 'uni' | 'soc'): Promise<{
    ...
    try {
      ...
      const files = fs
        .readdirSync(dataDir)
        .filter((file) => {
          const isKafaStatFile =
            file.startsWith('kafa-') &&
            file.endsWith('.json') &&
  ...
  /**
   * 패싱 스탯을 여러 정렬 방식으로 크롤링하여 모든 선수 수집
   */
  async scrapePassingWithAllSorts(league: 'uni' | 'soc') {
    const startTime = Date.now();
    this.logger.log(
      `🏈 ${league} 패싱 스탯 다중 정렬 크롤링 시작... (axios + cheerio)`,
    );
    try {
      // 1. 정렬 방식별 URL 매핑
      const sortMethods = [
      ...
      for (let i = 0; i < sortMethods.length; i++) {
        const sortMethod = sortMethods[i];
        this.logger.log(
          `📊 ${i + 1}/${sortMethods.length}: ${sortMethod.name} 정렬로 크롤링 중...`,
        );

        try {
          // URL 생성 (패싱은 ind_uni2.html)
          const url = `https://www.kafa.org/stats/ind_${league}2.html?mode=${sortMethod.mode}&year=${year}`;
            ...
            if (cells.length >= 8) {
              ...
              if (rankText && playerText) {
                ...
                if (playerMatch) {
                  ...
                  // 패싱 스탯 파싱 (KAFA 웹사이트 컬럼 순서에 맞게)
                  const passingYards = parseInt(
                    $(cells[2]).text().trim() || '0',
                  );
                  const yardsPerAttempt = parseFloat(
                    $(cells[3]).text().trim() || '0',
                  );
                  const completionPercentage = parseFloat(
                    $(cells[4]).text().trim() || '0',
                  );
                  const attempts = parseInt($(cells[5]).text().trim() || '0');
                  const completions = parseInt(
                    $(cells[6]).text().trim() || '0',
                  );
                  const touchdowns = parseInt($(cells[7]).text().trim() || '0');
                  const interceptions = parseInt(
                    $(cells[8]).text().trim() || '0',
                  );
                  const longest = parseInt($(cells[9]).text().trim() || '0');
          ...
          this.logger.log(
            `   ✅ ${sortMethod.name}: ${players.length}명 크롤링 완료`,
          );
          ...
          this.logger.error(
            `❌ ${sortMethod.name} 정렬 크롤링 실패: ${sortError.message}`,
          );
          sortResults.push({
            name: sortMethod.name,
            description: sortMethod.description,
      ...
      // 4. 결과 정리
      const uniquePlayers = Array.from(allPlayers.values());
      const totalCrawled = sortResults.reduce(
        (sum, result) => sum + result.players,
        0,
      );
      const duplicatesRemoved = totalCrawled - uniquePlayers.length;

      // 5. JSON 파일로 저장 (기존 패싱 스탯 파일 업데이트)
      const fileName = `kafa-${league}-passing.json`;
      const dataDir = path.join(process.cwd(), 'data');
      const filePath = path.join(dataDir, fileName);
      ...
      const jsonData = {
        ...
        players: uniquePlayers
          .map((player, index) => ({
            ...
            yardsPerAttempt: player.yardsPerAttempt,
            completionPercentage: player.completionPercentage,
            attempts: player.attempts,
          ...
          .sort((a, b) => b.passingYards - a.passingYards), // 패싱야드순 정렬
      };

      // JSON 파일 저장 (덮어쓰기)
      fs.writeFileSync(filePath, JSON.stringify(jsonData, null, 2), 'utf8');
      this.logger.log(
        `✅ JSON 파일 저장 완료: ${fileName} (${uniquePlayers.length}명)`,
      );

      // 6. 상위 샘플 데이터 (패싱야드순 정렬)
      const samplePlayers = uniquePlayers
      ...
      const result = {
        ...
        duplicatesRemoved,
        samplePlayers,
        allPlayersData: uniquePlayers,
      };

      this.logger.log(
        `🎉 패싱 다중 정렬 크롤링 완료: ${uniquePlayers.length}명 (중복 제거: ${duplicatesRemoved}명)`,
      );

      return result;
    } catch (error) {
      this.logger.error(`❌ 패싱 다중 정렬 크롤링 실패: ${error.message}`);
      throw new Error(`패싱 다중 정렬 크롤링 실패: ${error.message}`);
    }
  }

  // 리시빙 스탯 다중 정렬 크롤링 (50명 제한 극복)
  async scrapeReceivingWithAllSorts(league: 'uni' | 'soc') {
    const startTime = Date.now();
    this.logger.log(
      `🏈 ${league} 리시빙 스탯 다중 정렬 크롤링 시작... (axios + cheerio)`,
    );
    try {
      // 1. 정렬 방식별 URL 매핑
      const sortMethods = [
        { mode: 'rec', name: 'REC', description: '리시빙 횟수순' },
        { mode: 'yds', name: 'REC YDS', description: '리시빙 야드순' },
        { mode: 'avg', name: 'AVG', description: '리시빙당 야드순' },
        { mode: 'td', name: 'TD', description: '리시빙 터치다운순' },
        { mode: 'lng', name: 'LNG', description: '최장 리시빙순' },
      ];
      ...
      for (let i = 0; i < sortMethods.length; i++) {
        ...
        try {
          // URL 생성 (리시빙은 ind_uni3.html)
          const url = `https://www.kafa.org/stats/ind_${league}3.html?mode=${sortMethod.mode}&year=${year}`;
          this.logger.log(`   🔗 URL: ${url}`);
            ...
            if (cells.length >= 6) {
              ...
              if (rankText && playerText) {
                ...
                if (playerMatch) {
                  ...
                  // 리시빙 스탯 파싱 (KAFA 웹사이트 컬럼 순서에 맞게)
                  const receptions = parseInt($(cells[2]).text().trim() || '0');
                  const receivingYards = parseInt(
                    $(cells[3]).text().trim() || '0',
                  );
                  const yardsPerReception = parseFloat(
                    $(cells[4]).text().trim() || '0',
                  );
      ...
      // 5. JSON 파일로 저장
      const fileName = `kafa-${league}-receiving.json`;
      const dataDir = path.join(process.cwd(), 'data');
      const filePath = path.join(dataDir, fileName);
      ...
      this.logger.log(`💾 리시빙 데이터 저장 완료: ${filePath}`);

      const processingTime = ((Date.now() - startTime) / 1000).toFixed(1);
      this.logger.log(
        `🎉 리시빙 다중 정렬 크롤링 완료! 처리시간: ${processingTime}s`,
      );
      ...
      const result = {
        ...
        uniquePlayers: uniquePlayers.length,
        duplicatesRemoved,
        samplePlayers: uniquePlayers.slice(0, 5),
        allPlayersData: uniquePlayers,
      ...
      this.logger.error(`❌ 리시빙 다중 정렬 크롤링 실패: ${error.message}`);
      throw new Error(`리시빙 다중 정렬 크롤링 실패: ${error.message}`);
    }
  }

  // 펌블 스탯 다중 정렬 크롤링 (50명 제한 극복)
  async scrapeFumblesWithAllSorts(league: 'uni' | 'soc') {
    const startTime = Date.now();
    this.logger.log(`🏈 ${league} 펌블 스탯 다중 정렬 크롤링 시작... (axios + cheerio)`);
    try {
      // 1. 정렬 방식별 URL 매핑
      const sortMethods = [
        { mode: 'ff', name: 'FF', description: '강제 펌블순' },
        { mode: 'fr', name: 'FR', description: '펌블 회수순' },
        { mode: 'td', name: 'TD', description: '펌블 회수 터치다운순' }
      ];
      ...
      for (let i = 0; i < sortMethods.length; i++) {
        const sortMethod = sortMethods[i];
        this.logger.log(`📊 ${i + 1}/${sortMethods.length}: ${sortMethod.name} 정렬로 크롤링 중...`);
        
        try {
          // URL 생성 (펌블은 ind_uni4.html)
          const url = `https://www.kafa.org/stats/ind_${league}4.html?mode=${sortMethod.mode}&year=${year}`;
          this.logger.log(`   🔗 URL: ${url}`);
          ...
          $('table.stats_table tr').each((index, row) => {
            const cells = $(row).find('td');
            if (cells.length >= 5) { // 펌블은 5개 이상 컬럼
              const rankText = $(cells[0]).text().trim();
              const playerText = $(cells[1]).text().trim();
              ...
              if (rankText && playerText) {
                ...
                if (playerMatch) {
                  ...
                  // 펌블 스탯 파싱 (KAFA 웹사이트 컬럼 순서에 맞게)
                  const forcedFumbles = parseInt($(cells[2]).text().trim() || '0');
                  const fumbleRecoveries = parseInt($(cells[3]).text().trim() || '0');
                  const fumbleRecoveryTDs = parseInt($(cells[4]).text().trim() || '0');
      ...
      // 5. JSON 파일로 저장
      const fileName = `kafa-${league}-fumbles.json`;
      const dataDir = path.join(process.cwd(), 'data');
      const filePath = path.join(dataDir, fileName);
      ...
      const jsonData = {
        ...
        players: uniquePlayers.map((player, index) => ({
          ...
          university: player.university,
          jerseyNumber: player.jerseyNumber,
          forcedFumbles: player.forcedFumbles,
          fumbleRecoveries: player.fumbleRecoveries,
          fumbleRecoveryTDs: player.fumbleRecoveryTDs
        }))
      };
      
      fs.writeFileSync(filePath, JSON.stringify(jsonData, null, 2));
      this.logger.log(`💾 펌블 데이터 저장 완료: ${filePath}`);
      
      const processingTime = ((Date.now() - startTime) / 1000).toFixed(1);
      this.logger.log(`🎉 펌블 다중 정렬 크롤링 완료! 처리시간: ${processingTime}s`);
      ...
      this.logger.error(`❌ 펌블 다중 정렬 크롤링 실패: ${error.message}`);
      throw new Error(`펌블 다중 정렬 크롤링 실패: ${error.message}`);
    }
  }

  // 태클 스탯 다중 정렬 크롤링 (50명 제한 극복)
  async scrapeTacklesWithAllSorts(league: 'uni' | 'soc') {
    const startTime = Date.now();
    this.logger.log(`🏈 ${league} 태클 스탯 다중 정렬 크롤링 시작... (axios + cheerio)`);
    try {
      // 1. 정렬 방식별 URL 매핑
      const sortMethods = [
      ...
      for (let i = 0; i < sortMethods.length; i++) {
        ...
        try {
          // URL 생성 (태클은 ind_uni5.html)
          const url = `https://www.kafa.org/stats/ind_${league}5.html?mode=${sortMethod.mode}&year=${year}`;
          this.logger.log(`   🔗 URL: ${url}`);
            ...
            if (cells.length >= 6) { // 태클은 6개 셀 (순위, 선수, ATT, SACK, SOLO, COMBO)
              ...
              if (rankText && playerText) {
                ...
                if (playerMatch) {
                  ...
                  // 태클 스탯 파싱 (KAFA 웹사이트 컬럼 순서에 맞게)
                  const totalTackles = parseInt($(cells[2]).text().trim() || '0');
      ...
      // 5. JSON 파일로 저장
      const fileName = `kafa-${league}-tackles.json`;
      const dataDir = path.join(process.cwd(), 'data');
      const filePath = path.join(dataDir, fileName);
      ...
      fs.writeFileSync(filePath, JSON.stringify(jsonData, null, 2));
      this.logger.log(`💾 태클 데이터 저장 완료: ${filePath}`);
      
      const processingTime = ((Date.now() - startTime) / 1000).toFixed(1);
      this.logger.log(`🎉 태클 다중 정렬 크롤링 완료! 처리시간: ${processingTime}s`);
      ...
      this.logger.error(`❌ 태클 다중 정렬 크롤링 실패: ${error.message}`);
      throw new Error(`태클 다중 정렬 크롤링 실패: ${error.message}`);
    }
  }

  // 인터셉션 스탯 다중 정렬 크롤링 (50명 제한 극복)
  async scrapeInterceptionsWithAllSorts(league: 'uni' | 'soc') {
    const startTime = Date.now();
    this.logger.log(`🏈 ${league} 인터셉션 스탯 다중 정렬 크롤링 시작... (axios + cheerio)`);
    try {
      // 1. 정렬 방식별 URL 매핑
      const sortMethods = [
      ...
      for (let i = 0; i < sortMethods.length; i++) {
        ...
        try {
          // URL 생성 (인터셉션은 ind_uni6.html)
          const url = `https://www.kafa.org/stats/ind_${league}6.html?mode=${sortMethod.mode}&year=${year}`;
          this.logger.log(`   🔗 URL: ${url}`);
          ...
          $('table.stats_table tr').each((index, row) => {
            const cells = $(row).find('td');
            if (cells.length >= 6) { // 인터셉션은 6개 컬럼 (순위, 선수, INT, TD, YDS, LNG)
              const rankText = $(cells[0]).text().trim();
              const playerText = $(cells[1]).text().trim();
              ...
              if (rankText && playerText) {
                ...
                if (playerMatch) {
                  ...
                  // 인터셉션 스탯 파싱 (KAFA 웹사이트 컬럼 순서에 맞게)
                  const interceptions = parseInt($(cells[2]).text().trim() || '0');
                  const interceptionTouchdowns = parseInt($(cells[3]).text().trim() || '0');
                  const interceptionYards = parseInt($(cells[4]).text().trim() || '0');
                  const longestInterception = parseInt($(cells[5]).text().trim() || '0');
      ...
      // 5. JSON 파일로 저장
      const fileName = `kafa-${league}-interceptions.json`;
      const dataDir = path.join(process.cwd(), 'data');
      const filePath = path.join(dataDir, fileName);
      ...
      const jsonData = {
        ...
        players: uniquePlayers.map((player, index) => ({
          ...
          jerseyNumber: player.jerseyNumber,
          interceptions: player.interceptions,
          interceptionTouchdowns: player.interceptionTouchdowns,
          interceptionYards: player.interceptionYards,
          longestInterception: player.longestInterception
        }))
      };
      
      fs.writeFileSync(filePath, JSON.stringify(jsonData, null, 2));
      this.logger.log(`💾 인터셉션 데이터 저장 완료: ${filePath}`);
      
      const processingTime = ((Date.now() - startTime) / 1000).toFixed(1);
      this.logger.log(`🎉 인터셉션 다중 정렬 크롤링 완료! 처리시간: ${processingTime}s`);
      ...
      this.logger.error(`❌ 인터셉션 다중 정렬 크롤링 실패: ${error.message}`);
      throw new Error(`인터셉션 다중 정렬 크롤링 실패: ${error.message}`);
    }
  }

  // 필드골 스탯 다중 정렬 크롤링 (50명 제한 극복)
  async scrapeFieldGoalsWithAllSorts(league: 'uni' | 'soc') {
    const startTime = Date.now();
    this.logger.log(`🏈 ${league} 필드골 스탯 다중 정렬 크롤링 시작... (axios + cheerio)`);
    try {
      // 1. 정렬 방식별 URL 매핑
      const sortMethods = [
        { mode: 'fg%', name: 'FG%', description: '필드골 성공률순' },
        { mode: 'avg', name: 'AVG', description: '평균 야드순' },
        { mode: 'fgm', name: 'FGM', description: '필드골 성공 개수순' },
        { mode: 'att', name: 'ATT', description: '시도 횟수순' },
        { mode: 'yds', name: 'YDS', description: '총 야드순' },
      ...
      for (let i = 0; i < sortMethods.length; i++) {
        ...
        try {
          // URL 생성 (필드골은 ind_uni7.html)
          const url = `https://www.kafa.org/stats/ind_${league}7.html?mode=${sortMethod.mode}&year=${year}`;
          this.logger.log(`   🔗 URL: ${url}`);
          ...
          $('table.stats_table tr').each((index, row) => {
            const cells = $(row).find('td');
            if (cells.length >= 8) { // 필드골은 8개 이상 컬럼
              const rankText = $(cells[0]).text().trim();
              const playerText = $(cells[1]).text().trim();
              ...
              if (rankText && playerText) {
                ...
                if (playerMatch) {
                  ...
                  // 필드골 스탯 파싱 (KAFA 웹사이트 컬럼 순서에 맞게)
                  const fieldGoalPercentage = parseFloat($(cells[2]).text().trim() || '0');
                  const yardsAverage = parseFloat($(cells[3]).text().trim() || '0');
                  const fieldGoalsMade = parseInt($(cells[4]).text().trim() || '0');
                  const attempts = parseInt($(cells[5]).text().trim() || '0');
                  const totalYards = parseInt($(cells[6]).text().trim() || '0');
                  const longestFieldGoal = parseInt($(cells[7]).text().trim() || '0');
      ...
      // 5. JSON 파일로 저장
      const fileName = `kafa-${league}-fieldgoals.json`;
      const dataDir = path.join(process.cwd(), 'data');
      const filePath = path.join(dataDir, fileName);
      ...
      const jsonData = {
        ...
        players: uniquePlayers.map((player, index) => ({
          ...
          yardsAverage: player.yardsAverage,
          fieldGoalsMade: player.fieldGoalsMade,
          attempts: player.attempts,
          totalYards: player.totalYards,
          longestFieldGoal: player.longestFieldGoal
        }))
      };
      
      fs.writeFileSync(filePath, JSON.stringify(jsonData, null, 2));
      this.logger.log(`💾 필드골 데이터 저장 완료: ${filePath}`);
      
      const processingTime = ((Date.now() - startTime) / 1000).toFixed(1);
      this.logger.log(`🎉 필드골 다중 정렬 크롤링 완료! 처리시간: ${processingTime}s`);
      ...
      this.logger.error(`❌ 필드골 다중 정렬 크롤링 실패: ${error.message}`);
      throw new Error(`필드골 다중 정렬 크롤링 실패: ${error.message}`);
    }
  }

  // 킥오프 스탯 다중 정렬 크롤링 (50명 제한 극복)
  async scrapeKickoffsWithAllSorts(league: 'uni' | 'soc') {
    const startTime = Date.now();
    this.logger.log(`🏈 ${league} 킥오프 스탯 다중 정렬 크롤링 시작... (axios + cheerio)`);
    try {
      // 1. 정렬 방식별 URL 매핑
      const sortMethods = [
        { mode: 'avg', name: 'AVG', description: '평균 야드순' },
        { mode: 'ko', name: 'KO', description: '킥오프 횟수순' },
        { mode: 'yds', name: 'YDS', description: '총 야드순' },
      ...
      for (let i = 0; i < sortMethods.length; i++) {
        ...
        try {
          // URL 생성 (킥오프는 ind_uni8.html)
          const url = `https://www.kafa.org/stats/ind_${league}8.html?mode=${sortMethod.mode}&year=${year}`;
          this.logger.log(`   🔗 URL: ${url}`);
          ...
          $('table.stats_table tr').each((index, row) => {
            const cells = $(row).find('td');
            if (cells.length >= 7) { // 킥오프는 7개 이상 컬럼
              const rankText = $(cells[0]).text().trim();
              const playerText = $(cells[1]).text().trim();
              ...
              if (rankText && playerText) {
                ...
                if (playerMatch) {
                  ...
                  // 킥오프 스탯 파싱 (KAFA 웹사이트 컬럼 순서에 맞게)
                  const yardsAverage = parseFloat($(cells[2]).text().trim() || '0');
                  const kickoffs = parseInt($(cells[3]).text().trim() || '0');
                  const totalYards = parseInt($(cells[4]).text().trim() || '0');
                  const touchdowns = parseInt($(cells[5]).text().trim() || '0');
                  const longestKickoff = parseInt($(cells[6]).text().trim() || '0');
      ...
      // 5. JSON 파일로 저장
      const fileName = `kafa-${league}-kickoffs.json`;
      const dataDir = path.join(process.cwd(), 'data');
      const filePath = path.join(dataDir, fileName);
      ...
      const jsonData = {
        ...
        players: uniquePlayers.map((player, index) => ({
          ...
          jerseyNumber: player.jerseyNumber,
          yardsAverage: player.yardsAverage,
          kickoffs: player.kickoffs,
          totalYards: player.totalYards,
          touchdowns: player.touchdowns,
          longestKickoff: player.longestKickoff
        }))
      };
      
      fs.writeFileSync(filePath, JSON.stringify(jsonData, null, 2));
      this.logger.log(`💾 킥오프 데이터 저장 완료: ${filePath}`);
      
      const processingTime = ((Date.now() - startTime) / 1000).toFixed(1);
      this.logger.log(`🎉 킥오프 다중 정렬 크롤링 완료! 처리시간: ${processingTime}s`);
      ...
      this.logger.error(`❌ 킥오프 다중 정렬 크롤링 실패: ${error.message}`);
      throw new Error(`킥오프 다중 정렬 크롤링 실패: ${error.message}`);
    }
  }

  // 킥오프 리턴 스탯 다중 정렬 크롤링 (50명 제한 극복)
  async scrapeKickoffReturnsWithAllSorts(league: 'uni' | 'soc') {
    const startTime = Date.now();
    this.logger.log(`🏈 ${league} 킥오프 리턴 스탯 다중 정렬 크롤링 시작... (axios + cheerio)`);
    try {
      // 1. 정렬 방식별 URL 매핑
      const sortMethods = [
        { mode: 'avg', name: 'AVG', description: '평균 야드순' },
        { mode: 'ret', name: 'RET', description: '킥오프 리턴 횟수순' },
        { mode: 'yds', name: 'YDS', description: '총 리턴 야드순' },
        { mode: 'td', name: 'TD', description: '터치다운순' },
        { mode: 'lng', name: 'LNG', description: '최장 리턴순' }
      ];
      ...
      for (let i = 0; i < sortMethods.length; i++) {
        ...
        try {
          // URL 생성 (킥오프 리턴은 ind_uni9.html)
          const url = `https://www.kafa.org/stats/ind_${league}9.html?mode=${sortMethod.mode}&year=${year}`;
          this.logger.log(`   🔗 URL: ${url}`);
          ...
          $('table.stats_table tr').each((index, row) => {
            const cells = $(row).find('td');
            if (cells.length >= 7) { // 킥오프 리턴은 7개 이상 컬럼
              const rankText = $(cells[0]).text().trim();
              const playerText = $(cells[1]).text().trim();
              ...
              if (rankText && playerText) {
                ...
                if (playerMatch) {
                  ...
                  // 킥오프 리턴 스탯 파싱 (KAFA 웹사이트 컬럼 순서에 맞게)
                  const yardsAverage = parseFloat($(cells[2]).text().trim() || '0');
                  const kickoffReturns = parseInt($(cells[3]).text().trim() || '0');
                  const totalReturnYards = parseInt($(cells[4]).text().trim() || '0');
                  const touchdowns = parseInt($(cells[5]).text().trim() || '0');
                  const longestReturn = parseInt($(cells[6]).text().trim() || '0');
      ...
      // 5. JSON 파일로 저장
      const fileName = `kafa-${league}-kickoffreturns.json`;
      const dataDir = path.join(process.cwd(), 'data');
      const filePath = path.join(dataDir, fileName);
      ...
      const jsonData = {
        statType: 'kickoffreturns',
        statName: 'KICKOFF RETURNS',
        league: league,
        crawledAt: new Date().toISOString(),
        totalCount: uniquePlayers.length,
        players: uniquePlayers.map((player, index) => ({
          ...
          jerseyNumber: player.jerseyNumber,
          yardsAverage: player.yardsAverage,
          kickoffReturns: player.kickoffReturns,
          totalReturnYards: player.totalReturnYards,
          touchdowns: player.touchdowns,
          longestReturn: player.longestReturn
        }))
      };
      
      fs.writeFileSync(filePath, JSON.stringify(jsonData, null, 2));
      this.logger.log(`💾 킥오프 리턴 데이터 저장 완료: ${filePath}`);
      
      const processingTime = ((Date.now() - startTime) / 1000).toFixed(1);
      this.logger.log(`🎉 킥오프 리턴 다중 정렬 크롤링 완료! 처리시간: ${processingTime}s`);
      ...
      this.logger.error(`❌ 킥오프 리턴 다중 정렬 크롤링 실패: ${error.message}`);
      throw new Error(`킥오프 리턴 다중 정렬 크롤링 실패: ${error.message}`);
    }
  }

  // 펀트 스탯 다중 정렬 크롤링 (50명 제한 극복)
  async scrapePuntsWithAllSorts(league: 'uni' | 'soc') {
    const startTime = Date.now();
    this.logger.log(`🏈 ${league} 펀트 스탯 다중 정렬 크롤링 시작... (axios + cheerio)`);
    try {
      // 1. 정렬 방식별 URL 매핑
      const sortMethods = [
        { mode: 'avg', name: 'AVG', description: '평균 야드순' },
        { mode: 'punts', name: 'PUNTS', description: '펀트 횟수순' },
        { mode: 'yds', name: 'YDS', description: '총 야드순' },
        { mode: 'td', name: 'TD', description: '터치다운순' },
        { mode: 'lng', name: 'LNG', description: '최장거리순' }
      ];
      ...
      for (let i = 0; i < sortMethods.length; i++) {
        ...
        try {
          // URL 생성 (펀트는 ind_uni10.html)
          const url = `https://www.kafa.org/stats/ind_${league}10.html?mode=${sortMethod.mode}&year=${year}`;
          this.logger.log(`   🔗 URL: ${url}`);
          ...
          $('table.stats_table tr').each((index, row) => {
            const cells = $(row).find('td');
            if (cells.length >= 7) { // 펀트는 7개 이상 컬럼
              const rankText = $(cells[0]).text().trim();
              const playerText = $(cells[1]).text().trim();
              ...
              if (rankText && playerText) {
                ...
                if (playerMatch) {
                  ...
                  // 펀트 스탯 파싱 (KAFA 웹사이트 컬럼 순서에 맞게)
                  const yardsAverage = parseFloat($(cells[2]).text().trim() || '0');
                  const punts = parseInt($(cells[3]).text().trim() || '0');
                  const totalYards = parseInt($(cells[4]).text().trim() || '0');
                  const touchdowns = parseInt($(cells[5]).text().trim() || '0');
                  const longestPunt = parseInt($(cells[6]).text().trim() || '0');
      ...
      // 5. JSON 파일로 저장
      const fileName = `kafa-${league}-punts.json`;
      const dataDir = path.join(process.cwd(), 'data');
      const filePath = path.join(dataDir, fileName);
      ...
      const jsonData = {
        ...
        players: uniquePlayers.map((player, index) => ({
          ...
          totalYards: player.totalYards,
          touchdowns: player.touchdowns,
          longestPunt: player.longestPunt
        }))
      };
      
      fs.writeFileSync(filePath, JSON.stringify(jsonData, null, 2));
      this.logger.log(`💾 펀트 데이터 저장 완료: ${filePath}`);
      
      const processingTime = ((Date.now() - startTime) / 1000).toFixed(1);
      this.logger.log(`🎉 펀트 다중 정렬 크롤링 완료! 처리시간: ${processingTime}s`);
      ...
      this.logger.error(`❌ 펀트 다중 정렬 크롤링 실패: ${error.message}`);
      throw new Error(`펀트 다중 정렬 크롤링 실패: ${error.message}`);
    }
  }

  // 펀트 리턴 스탯 다중 정렬 크롤링 (50명 제한 극복)
  async scrapePuntReturnsWithAllSorts(league: 'uni' | 'soc') {
    const startTime = Date.now();
    this.logger.log(`🏈 ${league} 펀트 리턴 스탯 다중 정렬 크롤링 시작... (axios + cheerio)`);
    try {
      // 1. 정렬 방식별 URL 매핑
      const sortMethods = [
        { mode: 'avg', name: 'AVG', description: '평균 야드순' },
        { mode: 'ret', name: 'RET', description: '펀트 리턴 횟수순' },
        { mode: 'yds', name: 'YDS', description: '총 리턴 야드순' },
        { mode: 'td', name: 'TD', description: '터치다운순' },
        { mode: 'lng', name: 'LNG', description: '최장 리턴순' }
      ];
      ...
      for (let i = 0; i < sortMethods.length; i++) {
        ...
        try {
          // URL 생성 (펀트 리턴은 ind_uni11.html)
          const url = `https://www.kafa.org/stats/ind_${league}11.html?mode=${sortMethod.mode}&year=${year}`;
          this.logger.log(`   🔗 URL: ${url}`);
          ...
          $('table.stats_table tr').each((index, row) => {
            const cells = $(row).find('td');
            if (cells.length >= 7) { // 펀트 리턴은 7개 이상 컬럼
              const rankText = $(cells[0]).text().trim();
              const playerText = $(cells[1]).text().trim();
              ...
              if (rankText && playerText) {
                ...
                if (playerMatch) {
                  ...
                  // 펀트 리턴 스탯 파싱 (KAFA 웹사이트 컬럼 순서에 맞게)
                  const yardsAverage = parseFloat($(cells[2]).text().trim() || '0');
                  const puntReturns = parseInt($(cells[3]).text().trim() || '0');
                  const totalReturnYards = parseInt($(cells[4]).text().trim() || '0');
                  const touchdowns = parseInt($(cells[5]).text().trim() || '0');
                  const longestReturn = parseInt($(cells[6]).text().trim() || '0');
      ...
      // 5. JSON 파일로 저장
      const fileName = `kafa-${league}-puntreturns.json`;
      const dataDir = path.join(process.cwd(), 'data');
      const filePath = path.join(dataDir, fileName);
      ...
      const jsonData = {
        statType: 'puntreturns',
        statName: 'PUNT RETURNS',
        league: league,
        crawledAt: new Date().toISOString(),
        totalCount: uniquePlayers.length,
        players: uniquePlayers.map((player, index) => ({
          ...
          jerseyNumber: player.jerseyNumber,
          yardsAverage: player.yardsAverage,
          puntReturns: player.puntReturns,
          totalReturnYards: player.totalReturnYards,
          touchdowns: player.touchdowns,
          longestReturn: player.longestReturn
        }))
      ...
      fs.writeFileSync(filePath, JSON.stringify(jsonData, null, 2));
      this.logger.log(`💾 펀트 리턴 데이터 저장 완료: ${filePath}`);
      
      const processingTime = ((Date.now() - startTime) / 1000).toFixed(1);
      this.logger.log(`🎉 펀트 리턴 다중 정렬 크롤링 완료! 처리시간: ${processingTime}s`);
      ...
      this.logger.error(`❌ 펀트 리턴 다중 정렬 크롤링 실패: ${error.message}`);
      throw new Error(`펀트 리턴 다중 정렬 크롤링 실패: ${error.message}`);
    }
  }
  ...
  async scrapeRushingWithAllSorts(league: 'uni' | 'soc') {
    const startTime = Date.now();
    this.logger.log(
      `🏃‍♂️ ${league} 러싱 스탯 다중 정렬 크롤링 시작... (axios + cheerio)`,
    );
    ...
    try {
      ...
      for (let i = 0; i < sortMethods.length; i++) {
        const sortMethod = sortMethods[i];
        this.logger.log(
          `📊 ${i + 1}/5: ${sortMethod.name} 정렬로 크롤링 중...`,
        );
            ...
            if (cells.length >= 6) {
              ...
              if (rankText && playerText) {
                ...
                if (playerMatch) {
                  ...
                  // 러싱 야드에서 전진/후퇴 정보 파싱
                  const rushingYards = parseInt(
                    rushingYardsText.match(/^(-?\d+)/)?.[1] || '0',
                  );
      ...
      this.logger.log(
        `🎉 러싱 다중 정렬 크롤링 완료: ${uniquePlayers.length}명 (중복 제거: ${duplicatesRemoved}명)`,
      );
