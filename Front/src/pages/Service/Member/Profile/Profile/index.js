...
export default function ProfilePage() {
  ...
  const getSelectedTeam = () => {
    if (!profileData?.team) {
      return { name: 'N/A', logo: null, region: null, division: null };
    }

    const raw = String(profileData.team).trim();

    // 1️⃣ ID로 직접 매칭 (가장 우선)
    const byId = TEAM_BY_ID[raw];
    if (byId) return byId;

    // 3️⃣ 실패 시 기본값 반환
    return { name: raw, logo: null, region: null, division: null };
  };
